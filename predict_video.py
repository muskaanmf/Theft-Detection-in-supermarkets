...
# python predict_video.py -m model.hdf5 -l label_bin -i ./data2.mp4 -o new.mp4
...
while True:
	...
	if (preds[prediction]) < th:
		text = "{} - {:.2f}%".format((ok), 100 - (maxprob * 100))    # for normal
		cv2.putText(output, text, (35, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.25, (0,0,0), 5)
	else:
		fi_label = np.append(fi_label, label)
		text = "Alert : {} - {:.2f}%".format((label), maxprob * 100)
		cv2.putText(output, text, (35, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.25, (128,0,0), 5)
#		
		prelabel = label
...
with open('prelabel.pickle', 'wb') as f:
